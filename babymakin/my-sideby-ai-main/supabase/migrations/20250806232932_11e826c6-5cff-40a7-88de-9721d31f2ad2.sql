...
-- Create policies for email send logs (admins can view all)
DROP POLICY IF EXISTS "Admins can view email logs" ON public.email_send_logs;
CREATE POLICY "Admins can view email logs" 
ON public.email_send_logs 
FOR SELECT 
USING (public.is_admin_user());

DROP POLICY IF EXISTS "Email logs are insertable by system" ON public.email_send_logs;
CREATE POLICY "Email logs are insertable by system" 
ON public.email_send_logs 
FOR INSERT 
WITH CHECK (true);
