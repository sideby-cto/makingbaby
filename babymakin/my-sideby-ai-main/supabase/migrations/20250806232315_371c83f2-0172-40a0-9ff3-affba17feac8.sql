-- Create email templates and accounts tables if they don't exist
...
-- Create email send logs table for analytics
...
VALUES ('default', 'robot@sideby.ai', 'sideby', 'hello@sideby.ai')
...
-- Create policies for email accounts (read-only for authenticated users, write for admins)
CREATE POLICY "Email accounts are viewable by authenticated users" 
ON public.email_accounts 
FOR SELECT 
USING (auth.uid() IS NOT NULL);

CREATE POLICY "Email accounts are manageable by admins" 
ON public.email_accounts 
FOR ALL 
USING (public.is_admin_user());
