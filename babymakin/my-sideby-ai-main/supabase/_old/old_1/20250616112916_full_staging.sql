...
BEGIN
  IF NOT EXISTS (
    SELECT 1
      ...
      JOIN pg_namespace n ON n.oid = t.typnamespace
     WHERE n.nspname = 'public'
       AND t.typname = 'email_account_type'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
      ...
      JOIN pg_namespace n ON n.oid = t.typnamespace
     WHERE n.nspname = 'public'
       AND t.typname = 'feature_flag_type'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
      ...
      JOIN pg_namespace n ON n.oid = t.typnamespace
     WHERE n.nspname = 'public'
       AND t.typname = 'match_analysis_type'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
      ...
      JOIN pg_namespace n ON n.oid = t.typnamespace
     WHERE n.nspname = 'public'
       AND t.typname = 'session_frequency'
  ) THEN
...
-- testing.admin_role (fixed value list!)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
...
DROP FUNCTION IF EXISTS public.get_email_template_with_account(text) CASCADE;

CREATE OR REPLACE FUNCTION "public"."get_email_template_with_account"("template_key_param" "text") RETURNS TABLE("template_id" "uuid", "template_name" "text", "subject" "text", "header_html" "text", "body_html" "text", "footer_html" "text", "variables" "jsonb", "from_email" "text", "from_name" "text")
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
  RETURN QUERY
